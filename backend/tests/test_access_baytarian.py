...
Covers: vet_free visibility (instructor-only), baytarian lock + purchase gate,
free enroll, general purchase gate, baytarian request -> admin approve -> access.
"""
import io
import uuid
...
def demo():
    ...
    # ---- listing visibility: vet_free hidden from anonymous + students, shown to instructors ----
    ...
    assert vet_slug in ins_slugs, "vet_free must be visible to instructors"
    ...
    # ---- free enroll works; vet_free enroll blocked for student, allowed for instructor ----
    ...
    assert r.status_code == 403 and r.get_json()["error"] == "instructors_only", r.get_json()
    assert c.post("/api/v1/enrollments", json={"course_id": vet_id}, headers=ih).status_code == 201

    # ---- general purchasable by anyone; baytarian purchase blocked until verified ----
    ...
    # ---- now baytarian purchase gate passes + course unlocked ----
